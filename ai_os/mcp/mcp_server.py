...
async def dispatch_tool_call(ctx: ToolContext, name: str, arguments: dict[str, object] | None) -> types.CallToolResult:
    ...
    crashes the server.
    """
    from ai_os.mcp import git_tools  # local import to avoid circular at module load
    ...
    try:
        ...
        # Safe Git MCP tools (doc 22)
        # If caller passes an explicit repo_path, validate it stays within worktree.
        if name in ("git_status", "git_pull_main", "git_create_branch", "git_diff_summary"):
            ...
            if name == "git_status":
                result = git_tools.git_status(repo_path=effective_path)
                return _text_result(json.dumps(result))
            if name == "git_pull_main":
                result = git_tools.git_pull_main(repo_path=effective_path)
                return _text_result(json.dumps(result))
            if name == "git_create_branch":
                if "branch_name" not in filtered_args:
                    return _error_result("Missing required argument 'branch_name'")
                result = git_tools.git_create_branch(repo_path=effective_path, **filtered_args)
                return _text_result(json.dumps(result))
            if name == "git_diff_summary":
                result = git_tools.git_diff_summary(repo_path=effective_path, **filtered_args)
                return _text_result(json.dumps(result))
