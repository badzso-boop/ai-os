...
class DesignBrief:
    ...
    Fields:
        title: Startup name / title of the design brief.
        value_proposition: One-sentence value proposition or summary.
        target_audience: Target audience description.
        pages: List of key pages / views in the app.
        core_flow: Key steps in the main interactive user flow.
        brand: Brand guidelines, tone, aesthetic style.
        sim_entities: List of mock data model entities to simulate.
        non_requirements: Features explicitly out of scope.
        raw_prompt: Original raw text input prompt.
    """
    ...
    def __init__(
        ...
        brand: str = "",
        sim_entities: Optional[List[str]] = None,
        non_requirements: Optional[List[str]] = None,
        raw_prompt: str = "",
        ...
        self.value_proposition = value_prop if value_prop is not None else value_proposition
        self.target_audience = target_audience
        self.pages = list(pages) if pages is not None else []
        self.core_flow = list(core_flow) if core_flow is not None else []
        self.brand = brand_tone if brand_tone is not None else brand
        self.sim_entities = list(sim_entities) if sim_entities is not None else []
        self.non_requirements = list(non_requirements) if non_requirements is not None else []
    ...
    @value_prop.setter
    def value_prop(self, val: str) -> None:
        self.value_proposition = val

    @property
    def brand_tone(self) -> str:
        return self.brand

    @brand_tone.setter
    def brand_tone(self, val: str) -> None:
        self.brand = val
    ...
    def from_dict(cls, data: Dict[str, Any]) -> DesignBrief:
        ...
        return cls(
            ...
            value_proposition=str(data.get("value_proposition") or data.get("value_prop") or ""),
            target_audience=str(data.get("target_audience", "")),
            pages=list(data.get("pages", [])),
            core_flow=list(data.get("core_flow", [])),
            brand=str(data.get("brand") or data.get("brand_tone") or ""),
...
def parse_startup_brief(
    text: Union[str, Path, None] = None,
    source: Union[str, Path, None] = None,
) -> DesignBrief:
    """Parse a startup design brief from a file path or raw text string.

    Args:
        text: Raw text string or Path object containing brief content.
        source: Backward-compatible alias for text.

    Returns:
        Structured DesignBrief object.
    """
    src = text if text is not None else source
    if src is None:
        src = ""
