...
def test_design_brief_field_aliases_and_explicit_init():
    """Test initializing DesignBrief with explicit fields title, value_proposition, brand, sim_entities."""
    brief = DesignBrief(
        title="AcmeAI",
        value_proposition="Automate everything",
        target_audience="DevOps engineers",
        pages=["Home", "Settings"],
        core_flow=["Login", "Configure pipeline"],
        brand="Minimal dark mode",
        sim_entities=["Pipeline", "Job"],
    )

    assert brief.title == "AcmeAI"
    assert brief.name == "AcmeAI"
    assert brief.value_proposition == "Automate everything"
    assert brief.value_prop == "Automate everything"
    assert brief.target_audience == "DevOps engineers"
    assert brief.pages == ["Home", "Settings"]
    assert brief.core_flow == ["Login", "Configure pipeline"]
    assert brief.brand == "Minimal dark mode"
    assert brief.brand_tone == "Minimal dark mode"
    assert brief.sim_entities == ["Pipeline", "Job"]
...
def test_edge_cases_empty_and_minimal_inputs():
    ...
    assert empty_brief.title == "Untitled Startup"
    ...
    assert minimal_brief.name == "Untitled Startup" or minimal_brief.title == "Just a raw unformatted text prompt about an AI tool."
